...
export class ConfigModalComponent {
  ...
  private formatConfigCode(code: string): string {
    // Ako je prazan, vrati prazan string
    if (!code) return '';
    
    // Ako već sadrži "config: AnalogClockConfig", izdvoj JSON deo
    let jsonString = code;
    ...
    try {
      // Parse i format sa proper stilom
      const config = JSON.parse(jsonString);
      const formatted = JSON.stringify(config, null, 2)
        .replace(/"([^"]+)":/g, '$1:') // Ukloni navodnike oko keys
        .replace(/: "([^"]+)"/g, ": '$1'") // Zameni double quotes sa single quotes za string values
        .replace(/true/g, 'true')
      ...
      // Ako nije validan JSON, vrati originalni kod
      return code;
    }
  }
}